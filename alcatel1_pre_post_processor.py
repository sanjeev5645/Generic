...
class Alcatel_post_mac():
    def post(self,data):
        new_list = []
        for i in range(0,len(data)):
            new_dict = {}
            for key in data[i]:
                 if (key == 'VLAN' or key == 'Mac Address' or key == 'switchPort'):
                     new_dict.update({key:data[i][key]})
            new_list.append(new_dict)
        return(new_list)

class AlcatelPostRouterInterface(PrePostProcessor):

    def post_process(self, data):
        for item in data:
            item['ipAddress'] = item['ipAddress'] + '/'+ str(IPAddress(item['Subnet Mask']).netmask_bits())
            if (item['operationalStatus'] == 'YES'):
                item['operationalStatus'] = 'UP'
                item.update({'connected':'TRUE'})
            else:
                item['operationalStatus'] = 'DOWN'
                item.update({'connected': 'FALSE'})
            if 'vrf' not in item.keys():
                item.update({'vrf':'default'})
...
class AlcatelPostRoute(PrePostProcessor):

    def post_process(self,data):

        interface_file = open('/Users/sanjeevsathish/Desktop/Sanjeev/vRNI/UANI/network-insight-sdk-generic-datasources/Alcatel/ROUTER_INTERFACE1/router-interfaces1.csv')
        interface_list = list(csv.reader(interface_file))
        #print ( interface_list)

        for item in data:
            item.update({'vrf':'default'})
            item.update({'name':item['network']})
            item.pop('age')
            ...
            for i in range(1,len(interface_list)):
                if ( y == 1):
                    break

                host4 = ipaddress.ip_interface(interface_list[i][1])
                #print(interface_list[i][1])
                if (str(host4.network) == '0.0.0.0/32' or str(host4.network) == '0.0.0.0/0' ):
                    continue

                net4 = ipaddress.ip_network(host4.network)

                if (str(host4.ip) == item['nextHop']):
                    item.update({'interfaceName': interface_list[i][0]})
                    break

                for x in net4.hosts():
                    #print(str(x))

                    if ( str(x) == item['nextHop']):
                        #print("match")
                        #print(interface_list[i][0])
                        item.update({'interfaceName': interface_list[i][0]})
                        y=1
                        break
    ...
    def pre_process(self,txt):
        #print('****ROUTE PRE ***')
        #print(txt)
        re1 = re.compile(r'\d*d\s\d*h')
        txt1 = re1.sub('xxx', txt)
        #print('****ROUTE PRE ***')
        #print(txt1)
        return (txt1)


class AlcatelPostSwitchPort(PrePostProcessor):
        ...
        for dic1 in data:
            for key, value in dic1.items():
                if (key == 'connected'):
                    ...
                    if (value == 'up' or value == 'down' or value == 'Full' or value == 'Half'):
                        dic1[key] = value.upper()
        return (data)
